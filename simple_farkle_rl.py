...
def train_loop(action_reward: SimpleActionReward, action_opt: torch.optim.Optimizer, action_loss: torch.nn.Module,
               distance_scale: float, advantage_scale: float, epsilon: float, gamma: float, step: int) ->\
    ...
    while max(player_1_score, player_2_score) < 10000:
        ...
        while True:
            _, roll_maxes, remaining_dice, farkled, legal_moves = roll_loop(num_dice)
            state = [distance, advantage, current_turn_score, rolls] + roll_maxes
            action, roll_maxes = choose_action(farkled, num_dice, current_turn_score, current_score, roll_maxes,
                                               epsilon, step, legal_moves, action_reward, state)

            roll_score = roll_maxes[action - 1]
            actual_reward = compute_reward(distance_scale, advantage_scale, gamma, rolls, roll_score, current_score,
                                           advantage)
            states.append(state)
        ...
        if len(states) > 0:
            ...
            predicted_rewards = action_reward.forward(torch.tensor(states).float()).gather(
                1,torch.tensor(actions).unsqueeze(1) - 1)
...
def choose_action(farkled: bool, num_dice: int, current_turn_score: int, current_score: int, roll_maxes: list[int],
                  ...
                  state: list[int]) -> tuple[int, list[int]]:
    if farkled:
        action = num_dice
        ...
        action_reward.eval()
        with torch.no_grad():
            predicted_rewards = action_reward.forward(torch.tensor(state).float()).gather(0,
                                                                                          torch.tensor(legal_moves) - 1)
            probs = softmax(normalize(predicted_rewards, 1, 0), 0).tolist()
            action = random.choices(legal_moves, probs)[0]
    return action, roll_maxes


def compute_reward(distance_scale: float, advantage_scale: float, gamma: float, num_rolls: int, roll_score: int,
    ...
    distance_factor = distance_scale ** (current_score / 10000) # Decrease as closer to 10,000.
    advantage_factor = 1
    if advantage < 0:
        advantage_factor *= max(np.emath.logn(advantage_scale, -advantage), 1) # Increase as more behind.
...
def train() -> None:
    ...
    action_reward.load_state_dict(torch.load("simple_action_reward_state_dict_" + str(study.best_trial.number) + ".pt",
                                  weights_only=True))
    ...
    for _ in trange(0, 5000):
        _, player_1_turns, player_2_turns = simulate_game(strategy, strategy, True)
...
def objective(trial: optuna.Trial) -> float:
    ...
    for _ in trange(0, 1000):
        action_reward, action_opt = train_loop(action_reward, action_opt, action_loss, distance_scale, advantage_scale,
                                               epsilon, gamma, step)
        step += step_increment

    torch.save(action_reward.state_dict(), "simple_action_reward_state_dict_" + str(trial.number) + ".pt")
    ...
    ci_lower_1 = st.t.interval(0.95, len(turns_1) - 1, np.mean(turns_1), st.sem(turns_1))[0]
    ci_lower_2 = st.t.interval(0.95, len(turns_2) - 1, np.mean(turns_2), st.sem(turns_2))[0]
